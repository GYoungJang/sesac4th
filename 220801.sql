...
ALTER TABLE member Alter COLUMN gender SET default '여'; -- default 값을 바꿀 때에는 Alter set default
...
ALTER TABLE member MODIFY COLUMN interest varchar(100) AFTER promotion; -- 열 순서 변경하기 자료형까지 적어주어야 함..

INSERT INTO user (ID, name, birthday) VALUES ('id1', '홍길동', '2022-08-01'); -- 행 삽입
...
SELECT * FROM player WHERE position IN ('striker', 'goalkeeper'); -- == OR

-- LIKE

SELECT * FROM user WHERE name LIKE '%리';

UPDATE user SET name = '홍길동' WHERE ID != ''; -- 행 수정

UPDATE user SET name = '홍길동', birthday = '2022-08-01' WHERE ID != 'id2'; -- 행 수정
